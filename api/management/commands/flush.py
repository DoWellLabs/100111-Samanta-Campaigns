...
from ..utils import (
    ...
    flush_dbobjects_in_app,
    flush_dbobjects_in_apps
)
...
class Command(BaseCommand):
    ...
    """
    help = "Flushes DBObjects defined in `dbobjects.py` in a given app from the database"
    ...
    def handle(self, *args, **options):
        ...
        if flush_all_apps and app_label:
            self.stdout.write(
        ...
        if wants_interactive:
            self.stdout.write(
                "You have requested to flush DBObjects in {}.\n"
                "This will clear tables or collections of DBObjects in the database.\n\n"
                "Are you sure you want to do this?\n\n"
                "Type 'yes' to continue, or 'no' to cancel: ".format(app_label or ', '.join(apps_with_dbobjects_labels))
            )
            if input().lower() != "yes":
                self.stdout.write("Flush operation cancelled")
                return
            self.stdout.write("\n")

        if flush_all_apps:
            flush_dbobjects_in_apps(apps_with_dbobjects_labels, output_stream=self.stdout)
        else:
            flush_dbobjects_in_app(app_label, output_stream=self.stdout)
        return
